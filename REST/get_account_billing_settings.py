...
"""
----------------------------------------------------------------------
 Get account billing settings via REST API
----------------------------------------------------------------------
...
for account in accounts:
    ...
    # Split account into acctid and acctname if multiple fields 
    # have been provided in the accounts file
    #
    a = account.split(",")
    if a[0] != account:
        acctid = a[0]
    ...
    print(f"Requesting details for account ID {acctid}", file=sys.stderr)

    jData = json.loads(functions.get_data(
        settings["resturl"] + "/accounts/" + acctid + "/billingSettings" + fields,
        settings["username"], 
        settings["apikey"], 
        {}, 
        debug=args.debug))
